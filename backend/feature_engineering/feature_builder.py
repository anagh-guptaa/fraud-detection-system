...
def build_transaction_features(db, transaction_data):
    ...
    if not user_transactions:
        # ── Bug Fix #5: use a single aggregation query instead of loading the
        #    entire transactions table into Python memory (O(N) full scan) ──
        agg = db.query(
            func.avg(Transaction.amount),
            func.max(Transaction.amount),
        ).one()

        global_avg_amount = float(agg[0]) if agg[0] is not None else current_amount
        global_max_amount = float(agg[1]) if agg[1] is not None else current_amount
        amount_ratio = current_amount / global_avg_amount if global_avg_amount else 1.0

        return {
            "amount": current_amount,
            "avg_amount": round(global_avg_amount, 2),
            "max_amount": round(global_max_amount, 2),
